...
def get_min_in_day(data):
    ...
    for column in data.keys()[1:]:
        ...
        min_in_day_values[column+"_min"] = data[column][last_index]

    for i in data.index.tolist():
        day = data.loc[i]["Date"].split()[0]

        for column in min_in_day_values.keys():
            value = data.loc[i][column[:-4]]
            last_min_value = min_in_day_values[column]

            min_in_day_values[column] = search_min(value, last_min_value)

        result_data[day] = min_in_day_values
...
def get_max_in_day(data):
    ...
    for column in data.keys()[1:]:
        # last_index = len(data.keys()) - 2
        last_index = data.index[-1]
        max_in_day_values[column+"_max"] = data[column][last_index]

    for i in data.index.tolist():
        day = data.loc[i]["Date"].split()[0]

        for column in max_in_day_values.keys():
            value = data.loc[i][column[:-4]]
            last_max_value = max_in_day_values[column]

            max_in_day_values[column] = search_max(value, last_max_value)

        result_data[day] = max_in_day_values
